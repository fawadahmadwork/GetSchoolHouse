...
class PodsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pod" do
    assert_difference("Pod.count") do
      post pods_url, params: { pod: { address: @pod.address, grade: @pod.grade, name: @pod.name } }
    end
  ...
  test "should update pod" do
    patch pod_url(@pod), params: { pod: { address: @pod.address, grade: @pod.grade, name: @pod.name } }
